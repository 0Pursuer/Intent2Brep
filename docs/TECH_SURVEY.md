...
| Vector DrawingIR | lines/arcs/circles/hidden edges/dimensions | SVG/DXF; Ortho2CAD drawing pipeline; 2023 rule-based orthographic reconstruction assumes vectorized SVG | v0.1 stores SVG + manifest; explicit entity-level DrawingIR is v0.2 | Medium-high |
| Cross-view matching | identify corresponding projected vertices/edges | Zhang et al. 2023 pattern matching: SVG views -> 3D edge network | v0.2 research module | Medium for constrained drawings |
| 3D wireframe -> surfaces | loop detection, planar/cylindrical/etc surface hypotheses | classical CAD reconstruction; analytic geometry; BrepGaussian shows learned multi-view-to-parametric B-Rep direction | v0.1 bypasses this by compiling solved semantic primitives directly; v0.2 will add hypothesis engine | Medium; hard in ambiguous/general cases |
